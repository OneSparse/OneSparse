...
Datum scalar_row(PG_FUNCTION_ARGS)
{
    bytea          *data;
    Oid             rettype;
    TupleDesc       tupdesc;
    ...
    /* 1) get the requested return type from call context (anchored by anyelement arg) */
    ...
    for (int i = 0; i < natts; i++)
    {
        ...
        switch (layouts[i].kind)
        {
            ...
            case FK_BPCHAR:
            {
                /* Our packed bytes are a raw n-byte char[]; turn it into bpchar via input. */
                int   ndecl;
                int   n = (int) layouts[i].size;
                char *tmp;
                int   copylen = n;

                /* Treat trailing zero bytes as padding -> trim for cstring */
            ...
            case FK_BITBYTES:
            {
                /* Convert raw bytes back to a '0'/'1' string using nbits from typmod */
                int32 nbits = a->atttypmod;  /* for bit/varbit typmod is the bit length */
                char *bitstr;
